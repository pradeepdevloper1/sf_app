...
namespace XF.BASE
{
    public class CircleCountdown : SKCanvasView
    {
        ...
        private void DrawProgressCircle(SKImageInfo info, SKCanvas canvas)
        {
            ...
            // empty border
            var shader1 = SKShader.CreateSweepGradient(
                new SKPoint(size / 2, size / 2),
                new[]
                {
                    Color.FromHex("#ECF0F5").ToSKColor(),
                    Color.FromHex("#ECF0F5").ToSKColor(),
                    Color.FromHex("#ECF0F5").ToSKColor()
                },
                new[]
                {
                    StartAngle / 360,
                    (StartAngle + SweepAngle + 1) / 360,
                    (StartAngle + SweepAngle + 2) / 360
                });
            ...
            DrawCircle(info, canvas, paint1, SweepAngle);
