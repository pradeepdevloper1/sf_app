...
namespace XF.APP
{
    public class NavigationService : INavigationService
    {
        ...
        public async Task RemoveNPagesFromStack(int count)
        {
            var navigationStack = CurrentNavigationPage.Navigation;
            if (navigationStack.NavigationStack.Count > count)
            {
                do
                {
                    navigationStack.RemovePage(navigationStack.NavigationStack[navigationStack.NavigationStack.Count - count]);
                    count--;
                } while (count - 1 > 0);
            ...
            if (_navigationPageStack.Count > count)
            {
                var nav = _navigationPageStack.Peek().Navigation;
                nav.RemovePage(nav.NavigationStack[nav.NavigationStack.Count - count]);
                _navigationPageStack.Pop();
                await CurrentNavigationPage.Navigation.PopModalAsync();
                return;
            }
