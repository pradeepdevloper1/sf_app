...
export class EditFactoryComponent implements OnInit {
  IsfactoryDisabled: boolean = false;
  objForm: FormGroup;
  title = 'Create';
  data = false;
  UserForm: any;
  ...
  ngOnInit() {
    ...
    this.FactoryService.GetFactory(this.factoryid ).subscribe(
      res => {
        this.factorymodel = res;
        if (res.linkedwithERP == "YES") {
          this.IsfactoryDisabled = true;
        }
        console.log(res);
       ...
       this.organisationid = this.factorymodel.organisationID;
     
        this.clusterid = this.factorymodel.clusterID;

        this.factoryname = this.factorymodel.factoryName;
        this.factoryaddress = this.factorymodel.factoryAddress;
        ...
        this.factorytype = this.factorymodel.factoryType;


        this.linkedwithERP = this.factorymodel.linkedwithERP;
        ...
        this.countryid = this.factorymodel.factoryCountry;
        this.timezone = this.factorymodel.factoryTimeZone;
        this.noofline = this.factorymodel.noOfLine;
        this.smartlines = this.factorymodel.smartLines;
      })
  }
 ...
 linkedwitherpmodelChangeHandler(event: any) {
  ...
  UpdateFactory() {
    ...
    this.FactoryService.Update(this.factorymodel).subscribe(
      res => {
        /*        debugger;*/
        if (res.status == 200) {
          alert("Record save");
